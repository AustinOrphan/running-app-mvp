...
describe('useGoals', () => {
  ...
  describe('updateGoal', () => {
    it('successfully updates an existing goal', async () => {
      ...
      let resultGoal;
      await act(async () => {
        resultGoal = await result.current.updateGoal(existingGoal.id, { title: 'Updated Goal Title' });
      });

      expect(resultGoal).toEqual(updatedGoal);
      expect(result.current.goals.find(g => g.id === existingGoal.id)?.title).toBe('Updated Goal Title');
    });
  });
  ...
  describe('completeGoal', () => {
    it('successfully completes a goal', async () => {
      ...
      const completedGoal = { ...goalToComplete, isCompleted: true, completedAt: new Date().toISOString() };
  ...
  describe('Auto-completion', () => {
    it('auto-completes goals when progress indicates completion', async () => {
      ...
      const completedGoal = { ...incompleteGoal, isCompleted: true, completedAt: new Date().toISOString() };
