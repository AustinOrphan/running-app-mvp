...
describe('Goals API Integration Tests', () => {
  ...
  describe('GET /api/goals', () => {
    it('returns all active goals for authenticated user', async () => {
      ...
      // Create test goals using dynamic dates for independence
      const now = new Date();
      ...
      // Verify that all returned goals are active and belong to the user
      // Don't assume specific ordering - instead verify content
      const goalTitles = response.body.map((goal: any) => goal.title).sort();
      expect(goalTitles).toEqual(['Run 100km this month', 'Run 5 times per week']);

      response.body.forEach((goal: any) => {
        expect(goal).toHaveProperty('id');
        expect(goal).toHaveProperty('isActive', true);
        expect(goal).toHaveProperty('userId', user.id);
      });
    });
