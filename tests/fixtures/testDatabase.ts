...
import { getTestPrismaClient } from '../utils/connectionPoolManager';

import { mockRuns, mockTestUser, mockGoals, mockRaces } from './mockData';

// Use shared connection pool instead of creating new clients
...
export const generateTestToken = (userId: string, email?: string) => {
  ...
  const payload = {
    id: userId,
    email: email || testDataUtils.generateUniqueEmail('test'),
    type: 'access',
...
// Clean up database utility - optimized with transaction and connection pooling
export const cleanupDatabase = async () => {
  // Use transaction for atomic cleanup - faster and safer
  // Include timeout to prevent hanging in case of connection issues
  await prisma.$transaction(
    ...
    {
      timeout: 30000, // 30s timeout to prevent hanging tests
    }
  );
};
