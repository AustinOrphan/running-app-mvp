...
import { waitForAnimation } from './utils/wait-conditions';
    ...
    test('should handle keyboard navigation', async ({ page }) => {
      ...
      if (elementCount > 0) {
        ...
        for (let i = 0; i < Math.min(5, elementCount - 1); i++) {
          ...
          // Skip redundant animation wait // Small delay for focus changes
        }

        // Test Shift+Tab (reverse tabbing)
        await page.keyboard.press('Shift+Tab');
        // Skip redundant animation wait
      }
    });
  });
  ...
  test.describe('Mobile Navigation Tests', () => {
    test('should handle mobile navigation menu', async ({ page }) => {
      ...
      if (menuButton) {
        ...
        await waitForAnimation(page, 150); // Wait for menu animation (reduced from 300ms)
    ...
    test('should handle swipe gestures for navigation', async ({ page }) => {
      ...
      if (viewport) {
        ...
        await page.mouse.up();
        await waitForAnimation(page, 200); // Reduced from 500ms
    ...
    test('should handle pull-to-refresh gesture', async ({ page }) => {
      ...
      if (viewport) {
        ...
        await waitForAnimation(page, 300); // Reduced from 800ms
    ...
    test('should handle long press gestures', async ({ page }) => {
      ...
      if (itemCount > 0) {
        ...
        if (itemBox) {
          ...
          await waitForAnimation(page, 200); // Reduced from 400ms

          await waitForAnimation(page, 150); // Further reduced from 300ms
    ...
    test('should handle pinch-to-zoom gestures on compatible content', async ({ page }) => {
      ...
      if (elementCount > 0) {
        ...
        if (elementBox) {
          ...
          await waitForAnimation(page, 150); // Reduced from 300ms
