 ...
 */
export function generateRunNotes(): string {
  const templates = [
    'Felt {feeling} today. {weather} weather. {extra}',
    '{weather} conditions. {feeling} throughout the run. {extra}',
    'Great run! {feeling} and {weather}. {extra}',
    '{extra} {feeling} legs today. {weather} outside.',
  ];

  const feelings = ['great', 'good', 'okay', 'tired', 'strong', 'sluggish', 'energetic'];
  const weather = ['sunny', 'cloudy', 'windy', 'perfect', 'hot', 'cold', 'rainy'];
  const extras = [
    ...
    'Ran with friends.',
    'Hit all my splits!',
    'Tough hills but pushed through.',
    '',
  ];

  const template = faker.helpers.arrayElement(templates);
  return template
    .replace('{feeling}', faker.helpers.arrayElement(feelings))
    .replace('{weather}', faker.helpers.arrayElement(weather))
    .replace('{extra}', faker.helpers.arrayElement(extras));
}
