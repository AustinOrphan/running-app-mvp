...
export class InMemoryDatabase {
  ...
  constructor(config: Partial<InMemoryDbConfig> = {}) {
    ...
    const tempDbPath = platformUtils.createTempFilePath(`test-db-${this.config.id}`, 'db');
    this.databaseUrl = `file:${platformUtils.toUnixPath(tempDbPath)}`;
  }
  ...
  private async runMigrations(): Promise<void> {
    try {
      ...
      try {
        ...
        platformUtils.execCommand('npx prisma db push --force-reset', {
          stdio: this.config.enableLogging ? 'inherit' : 'pipe',
          env: {
            ...platformUtils.createTestEnvironment(),
  ...
  async destroy(): Promise<void> {
    try {
      ...
      if (this.databaseUrl.startsWith('file:')) {
        ...
        try {
          await fs.unlink(platformUtils.normalizePath(filePath));
        } catch {
