...
import { MOCK_DATES, formatters } from '../setup/globalDateMock';

/**
 * Common test date values for consistent testing
 * These are now based on the global mock date for consistency
 */
export const TEST_DATES = {
  // Standard test dates
  PAST_DATE: '2024-01-01',
  TODAY: MOCK_DATES.NOW_STRING, // Always uses mocked date
 ...
 * Note: This now returns the mocked date, not the actual current date
 *
 * @returns Current date formatted for input
 */
export const getCurrentDateForInput = (): string => {
  return formatters.toDateInput(new Date());
};
