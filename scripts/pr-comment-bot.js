...
class PRCommentBot {
  ...
  async collectCoverageResults() {
    try {
      ...
      try {
        const integrationSummaryPath = path.join(this.options.integrationCoverageDir, 'coverage-summary.json');
  ...
  parseVitestResults(output) {
    try {
      ...
      let jsonLine = lines.find(line => line.trim().startsWith('{') && line.includes('numTotalTests'));
      ...
      if (jsonLine) {
        ...
        return {
          ...
          failures: results.testResults?.filter(t => t.status === 'failed')?.map(t => ({
    ...
    try {
      ...
      if (jsonLine) {
        ...
        return {
          ...
          failed_tests: results.numFailedTests || 0,
          skipped_tests: results.numPendingTests || 0,
          failures: results.testResults?.flatMap(suite => 
    ...
    try {
      ...
      if (output.includes('"stats"') && output.includes('"expected"')) {
        ...
        return {
          ...
          failures: results.suites?.flatMap(suite => 
            suite.specs?.flatMap(spec =>
              spec.tests?.filter(test => test.results?.[0]?.status === 'failed')?.map(test => ({
...
**Generated**: ${new Date().toLocaleString()}`;
  ...
  generateTestResultsSummary() {
    ...
    const totalPassed = suites.reduce((sum, suite) => sum + (this.testResults[suite].passed_tests || 0), 0);
    const totalFailed = suites.reduce((sum, suite) => sum + (this.testResults[suite].failed_tests || 0), 0);
    const totalSkipped = suites.reduce((sum, suite) => sum + (this.testResults[suite].skipped_tests || 0), 0);
    const totalDuration = suites.reduce((sum, suite) => sum + (this.testResults[suite].duration || 0), 0);
  ...
  generateDetailedTestResults() {
    ...
    for (const suite of suites) {
      ...
      if (result.failed_tests > 0 && result.failures && result.failures.length > 0) {
        ...
        for (const failure of result.failures.slice(0, 5)) { // Limit to 5 failures
  ...
  generateCoverageSection() {
    ...
    if (this.testResults.coverage.overall) {
      ...
      for (const metric of metrics) {
        ...
        coverage.push(`| ${metric.charAt(0).toUpperCase() + metric.slice(1)} | ${value}% | ${status} |`);
      }
    ...
    if (this.testResults.coverage.unit || this.testResults.coverage.integration) {
      ...
      if (this.testResults.coverage.unit) {
        ...
        coverage.push(`| Unit Tests | ${unit.statements}% | ${unit.branches}% | ${unit.functions}% | ${unit.lines}% |`);
      }
      
      if (this.testResults.coverage.integration) {
        const integration = this.testResults.coverage.integration;
        coverage.push(`| Integration Tests | ${integration.statements}% | ${integration.branches}% | ${integration.functions}% | ${integration.lines}% |`);
      }
  ...
  generatePerformanceSection() {
    ...
    if (perf.suitePerformance && Object.keys(perf.suitePerformance).length > 0) {
      ...
      for (const [suite, data] of Object.entries(perf.suitePerformance)) {
        ...
        performance.push(`| ${suite} | ${this.formatDuration(data.duration)} | ${testsPerSec} | ${status} |`);
      }
  ...
  generateFlakyTestsSection() {
    ...
    if (flakyData.flakyTests && flakyData.flakyTests.length > 0) {
      ...
      for (const test of flakyData.flakyTests.slice(0, 10)) { // Limit to 10
