...
class FlakyTestTracker {
  ...
  async runTestsWithTracking(testSuite = 'all') {
    ...
    const suites = testSuite === 'all' ? ['unit', 'integration', 'e2e'] : [testSuite];
  ...
  async analyzeResults() {
    ...
    return {
      ...
      summary: {
        ...
        flakyPercentage:
          this.testHistory.size > 0
            ? ((flakyTests.length / this.testHistory.size) * 100).toFixed(2)
            : 0,
      },
    };
  }
...
`;
  ...
  generateRecommendations(analysis) {
    ...
    if (analysis.flaky.length === 0) {
      recommendations.push(
        '✅ Great! No flaky tests detected. Keep monitoring to maintain stability.'
      );
    } else {
      recommendations.push(
        `🔍 ${analysis.flaky.length} flaky tests detected. Prioritize investigation by severity.`
      );

      const highSeverity = analysis.flaky.filter(
        t => this.categorizeFlakyness(t.flakyScore) === 'high'
      );
      if (highSeverity.length > 0) {
        recommendations.push(
          `🚨 ${highSeverity.length} high-severity flaky tests need immediate attention.`
        );
      }

      recommendations.push(
        '🔧 Consider adding retry logic with exponential backoff for flaky tests.'
      );
    ...
    if (analysis.consistentlyFailing.length > 0) {
      recommendations.push(
        `❌ ${analysis.consistentlyFailing.length} tests are consistently failing - these should be fixed or disabled.`
      );
    }

    if (analysis.summary.flakyPercentage > 5) {
      recommendations.push(
        '📈 Flaky test percentage is high (>5%). Consider reviewing test infrastructure and isolation.'
      );
    }
  ...
  async generateRetryConfig() {
    ...
    const flakyTests = Array.from(this.flakyTests.values()).filter(
      test => test.flakyScore >= this.options.threshold
    );
