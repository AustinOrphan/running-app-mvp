...
class ErrorCodeExtractor {
  ...
  printTestingRecommendations() {
    ...
    prioritizedScenarios.forEach(([scenario, patterns], index) => {
      const uncoveredHandlers = patterns.filter(p => 
        p.type === 'catch-block' && !p.hasErrorHandling
      );
  ...
  getPriorityScenarios() {
    ...
    return Object.entries(scenarioGroups)
      .map(([scenario, patterns]) => ({
        ...
        uncoveredHandlers: patterns.filter(p => 
          p.type === 'catch-block' && !p.hasErrorHandling
        ).length
      }))
  ...
  getTestCandidates() {
    return this.errorPatterns
      .filter(pattern => 
        pattern.type === 'catch-block' && !pattern.hasErrorHandling
      )
      .map(pattern => ({
        file: pattern.file,
        line: pattern.line,
