...
class CacheManager {
  ...
  async generateCacheKey(type: 'unit' | 'integration' | 'e2e'): Promise<string> {
    ...
    const configHash = await this.hashFiles(
      ['.'],
      configFiles.map(f => path.basename(f))
    );
  ...
  async invalidateCache(type?: 'unit' | 'integration' | 'e2e'): Promise<void> {
    ...
    const typesToClear = type
      ? [type]
      : (Object.keys(cacheDirs) as Array<'unit' | 'integration' | 'e2e'>);
  ...
  async setupGitHubActionsCache(): Promise<void> {
    ...
    console.log(
      "    key: ${{ runner.os }}-tests-${{ hashFiles('**/*.test.ts', 'package-lock.json') }}"
    );
  ...
  async saveResultCache(
    type: 'unit' | 'integration' | 'e2e',
    results: { passed: boolean; duration?: number; coverage?: number }
  ): Promise<void> {
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        '  npm run cache invalidate <type>   - Clear specific cache (unit/integration/e2e)'
      );
      console.log('  npm run cache monitor             - View performance metrics');
      console.log('  npm run cache setup               - Show GitHub Actions setup');
      console.log(
        '  npm run cache check <type>        - Check if tests can be skipped due to cache'
      );
  }
}
