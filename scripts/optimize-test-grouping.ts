...
function groupTests(allTests: TestFile[]): TestGroup[] {
  ...
  const fastUnitTests = allTests.filter(t => 
    t.type === 'unit' && 
    t.parallelSafe && 
  ...
  const slowUnitTests = allTests.filter(t => 
    t.type === 'unit' && 
    t.parallelSafe && 
    t.estimatedDuration > 5
  );
  ...
  const parallelIntegrationTests = allTests.filter(t => 
    t.type === 'integration' && 
    t.parallelSafe
  );
  ...
  if (parallelIntegrationTests.length > 0) {
    groups.push({
      ...
      parallelSafe: true,
      workerRecommendation: Math.min(3, Math.max(1, Math.ceil(parallelIntegrationTests.length / 3))),
    });
  }
  
  // Group 5: Sequential integration tests (database, auth, etc.)
  const sequentialIntegrationTests = allTests.filter(t => 
    t.type === 'integration' && 
    !t.parallelSafe
  );
  ...
  if (sequentialIntegrationTests.length > 0) {
    groups.push({
      ...
      estimatedDuration: sequentialIntegrationTests.reduce((sum, t) => sum + t.estimatedDuration, 0),
      parallelSafe: false,
...
function generateOptimizationReport(groups: TestGroup[]): void {
  ...
  const maxParallelDuration = Math.max(...parallelGroups.map(g => 
    Math.ceil(g.estimatedDuration / g.workerRecommendation)
  ));
  ...
  console.log(`export JEST_WORKERS=${groups.find(g => g.name.includes('Integration'))?.workerRecommendation || 2}`);
  console.log('');
  console.log('# Set Playwright workers for E2E tests');
  console.log(`export PLAYWRIGHT_WORKERS=${groups.find(g => g.name.includes('E2E'))?.workerRecommendation || 3}`);
  console.log('');
...
function generatePackageJsonScripts(groups: TestGroup[]): void {
  ...
  groups.forEach(group => {
    ...
    const workerConfig = group.parallelSafe ? `--maxWorkers=${group.workerRecommendation}` : '--maxWorkers=1';
    
    if (group.name.includes('Unit')) {
      console.log(`  "test:${scriptName}": "vitest run --pool=threads --poolOptions.threads.maxThreads=${group.workerRecommendation}",`);
    } else if (group.name.includes('Integration')) {
      console.log(`  "test:${scriptName}": "jest ${workerConfig} --testPathPattern=tests/integration",`);
    } else if (group.name.includes('E2E')) {
      console.log(`  "test:${scriptName}": "playwright test --workers=${group.workerRecommendation}",`);
    } else if (group.name.includes('Accessibility')) {
      console.log(`  "test:${scriptName}": "vitest run tests/accessibility --pool=threads --poolOptions.threads.maxThreads=${group.workerRecommendation}",`);
    }
  });
  
  console.log('  "test:parallel:safe": "npm run test:fast-unit-tests && npm run test:accessibility-tests && npm run test:parallel-integration-tests",');
  console.log('  "test:sequential:required": "npm run test:sequential-integration-tests",');
  console.log('  "test:optimized": "npm run test:parallel:safe && npm run test:sequential:required && npm run test:e2e-tests"');
  console.log('}');
  console.log('```');
}
