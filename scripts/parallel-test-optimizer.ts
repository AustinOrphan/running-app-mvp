...
class ParallelTestOptimizer {
  ...
  private async analyzeTestDirectory(directory: string, type: 'unit' | 'integration' | 'e2e'): Promise<void> {
    try {
  ...
  private async analyzeTestFile(filePath: string, type: 'unit' | 'integration' | 'e2e'): Promise<TestFile> {
    ...
    const resourceIntensive = /\b(large|bulk|performance|memory|cpu)\b/i.test(content) || 
                             content.includes('jest.setTimeout') || 
                             content.includes('test.setTimeout');
    
    // Determine if test is parallel-safe
    const parallelSafe = !databaseDependent || 
  ...
  async createOptimalGroups(): Promise<TestGroup[]> {
    ...
    const fastUnitTests = this.testFiles.filter(t => 
      t.type === 'unit' && 
      t.parallelSafe && 
      t.estimatedDuration < 2 && 
      !t.resourceIntensive
    );
    ...
    const slowUnitTests = this.testFiles.filter(t => 
      t.type === 'unit' && 
      (t.estimatedDuration >= 2 || t.resourceIntensive) &&
      t.parallelSafe
    );
    ...
    const dbIntegrationTests = this.testFiles.filter(t => 
      t.type === 'integration' && 
      t.databaseDependent
    );
    ...
    const apiIntegrationTests = this.testFiles.filter(t => 
      t.type === 'integration' && 
      !t.databaseDependent && 
      t.parallelSafe
    );
    ...
    const browserE2ETests = this.testFiles.filter(t => 
      t.type === 'e2e' && 
      t.browserDependent
    );
    ...
    const apiE2ETests = this.testFiles.filter(t => 
      t.type === 'e2e' && 
      !t.browserDependent
    );
  ...
  async generateOptimizationReport(groups: TestGroup[]): Promise<void> {
    ...
    if (slowestGroup.estimatedDuration > parallelDuration * 0.8) {
      console.log(`- Consider splitting "${slowestGroup.name}" further (${slowestGroup.estimatedDuration.toFixed(1)}s)`);
    }
    
    const sequentialGroups = groups.filter(g => g.maxParallelWorkers === 1);
    if (sequentialGroups.length > 0) {
      console.log(`- ${sequentialGroups.length} groups run sequentially - consider isolating dependencies`);
    }
    ...
    console.log(`- Total workers needed: ${groups.reduce((sum, g) => sum + g.maxParallelWorkers, 0)}`);
    console.log(`- Recommended CI runners: ${Math.ceil(groups.reduce((sum, g) => sum + g.maxParallelWorkers, 0) / 4)}`);
  }
  ...
  async validateCurrentConfiguration(): Promise<void> {
    ...
    try {
      ...
      if (vitestConfigContent.includes('maxThreads') || vitestConfigContent.includes('minThreads')) {
    ...
    try {
      ...
      if (vitestCiContent.includes('maxThreads')) {
        ...
        console.log(`   - CI Threads: ${maxThreadsMatch?.[1] || 'configured'} (CI) / ${maxThreadsMatch?.[2] || 'configured'} (local)`);
      }
    } catch (err) {
