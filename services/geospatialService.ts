...
export class GeospatialService {
   ...
   */
  static generateSplits(
    route: RunRoute,
    splitDistance: number = 1
  ): Array<{
    distance: number;
    time: number;
    pace: number;
    avgHeartRate?: number;
    elevation?: { gain: number; loss: number; max: number; min: number };
  }> {
   ...
   */
  static async findPopularRoutes(
    userId: string,
  ...
  static async createGridHeatmap(
    ...
    // Create interpolated grid from point collection
